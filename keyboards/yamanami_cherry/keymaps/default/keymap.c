...
layer_state_t layer_state_set_user(layer_state_t state) {
    state = update_tri_layer_state(state, _LOWER_UU, _RAISE_UU, _ADJUST_UU);
    state = update_tri_layer_state(state, _LOWER_UJ, _RAISE_UJ, _ADJUST_UJ);
    state = update_tri_layer_state(state, _LOWER_JU, _RAISE_JU, _ADJUST_JU);
    state = update_tri_layer_state(state, _LOWER_JJ, _RAISE_JJ, _ADJUST_JJ);

    if (get_highest_layer(state) == 0) {
        if (isDefaultLayer == 0) {
            isCurrentLayer = _QWERTY_UU;
        } else if (isDefaultLayer == 1) {
            isCurrentLayer = _QWERTY_UJ;
        } else if (isDefaultLayer == 2) {
            isCurrentLayer = _QWERTY_JU;
        } else {
            isCurrentLayer = _QWERTY_JJ;
        }
    } else {
